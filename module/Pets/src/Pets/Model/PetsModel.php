...
// 			->where->notIn("r_q.id_status", $arrayStatusArticles);
	...
	public function getAllPushNotifications($idUser)
	{
    	...
    	$select
			...
			//->group ( array ('a.id') )
			//->limit(1)
			->where(array('p_n.type_notification' => 2))
	...
	public function getBreedsByTypeOfPet($typePet)
	{
		...
		$select = $sql->select();
		$select
			->columns(array('id', 'name_breed', 'type_pet'))
			->from(array('breeds' => 'breeds_of_pet'))
			->where(array('breeds.type_pet' => $typePet))
			->order('breeds.name_breed ASC');
