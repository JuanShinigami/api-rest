...

  	{
		...
		//print($getBasePathFull ); exit;
  		...
  		for ($i=0; $i < $numberCodeQr ; $i++) {
			...
			$path_img_qr =  $protocol . $_SERVER['SERVER_NAME'] . $getBasePathFull . '/public/codigos_qr/images_qr/' . $securePseudoRandomBytes . '.svg';

			//echo "<pre>"; print_r($path_img_qr); exit;



			$pdf = new \TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, array('25.2','25.2'), true, 'UTF-8', false);
        	...
        	$pdf->ImageSVG($file=$path_img_qr, $x=-3, $y=-3, $w='', $h='', $link='', $align='', $palign='', $border=0, $fitonpage=false);
        	...
        	//print_r($_SERVER['DOCUMENT_ROOT'] . substr($getBasePathFull, 1) .'public/codigos_qr/pdf_qr/' . $securePseudoRandomBytes . '.pdf'); exit;

        	$pdf->Output($_SERVER['DOCUMENT_ROOT'] . substr($getBasePathFull, 1) .'public/codigos_qr/pdf_qr/' . $securePseudoRandomBytes . '.pdf', 'F');
        	...
        	$listCodesQR[] = array(
        		...
        		'root_pdf'    => $_SERVER['DOCUMENT_ROOT'] . substr($getBasePathFull, 1) .'/public/codigos_qr/pdf_qr/' . $securePseudoRandomBytes . '.pdf',
