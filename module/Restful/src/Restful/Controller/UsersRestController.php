    ...
    public function addAction()
    {
    	...
    	if ($request->isPost()) {
            ...
            if ($verifyEmail[0]['count'] == 1) {
                ...
                // CP
                $zipCode            = $decodePostData['postal_code'];

                // Obtemos direccion en base al cp
                $addressZipCode     = $this->getUsersService()->getAddressByZipCode($zipCode);
                //echo "<pre>"; print_r($addressZipCode); exit;

                // Estado
                $decodePostData['state_of_mexico']  = (count($addressZipCode) != 0) ? $addressZipCode[0]['state_id'] : 0;

                // Municipio / Delegacion
                $decodePostData['district']         = (count($addressZipCode) != 0) ? $addressZipCode[0]['district_id'] : 0;

                // Colonia
                $decodePostData['neighborhood']     = 0;
