...
class UsersService
{
	...
	private function createAndSaveAddressUsers($data, $addUser)
	{
		...
		//echo "<pre>"; print_r($addAddress); exit;

		return $addAddress;
	}

	/**
	 * MODIFICAR UNA DIRECCION PARA USUARIOS
	 */
	private function createAndUpdateAddressUsers($data, $idUser)
	{
		// Arreglo de datos
		$dataAddress = array(
			'id_users'  		=> $idUser,
		...
		// Modificamos direccion
		$updateAddress = $this->getAddressUsersService()->updateAddress($dataAddress);
	...
	/**
	 * OBTENER DIRECCION APARTIR DEL CODIGO POSTAL
	 */
	public function getAddressByZipCode($zip_code)
	{
		//echo "Obtener direccion a partir del CP"; exit;

		// Direccion
		$addressByZipCode = $this->getUsersModel()->getAddressByZipCode($zip_code);
...
/**
	...
	public function editProfileAppMovil($data)
	{
		...
		// ************************************************************
		// 				AGREGAR O MODIFICAR DIRECCCION
		// ************************************************************

		// Obtener direccion de usuario
		$getAddressUserById = $this->getAddressUsersService()->getAddressUserById($data['id']);
		//echo "<pre>"; print_r(count($getAddressUserById)); exit;

		// VALIDAR SI EXISTE DIRECCION
		if (count($getAddressUserById) != 0) {
			
			// Modificar direccion de un usuario
			$updateAddressUser = $this->createAndUpdateAddressUsers($data, $data['id']);
			//echo "<pre>"; print_r($updateAddressUser); exit;

		} else {

			// Agregar direccion
			$addAdressUser = $this->createAndSaveAddressUsers($data, $data['id']);
			//echo "<pre>"; print_r($addAdressUser); exit;
